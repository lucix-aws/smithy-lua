...
test("FlattenedXmlMapWithXmlName", function()
    local input = {
        myMap = {
        a = "A",
        b = "B",
    ...
    assert_url_path(request.url, "/FlattenedXmlMapWithXmlName")
    assert_header(request, "Content-Type", "application/xml")
    local body_str = read_body(request.body)
    assert_xml_eq(body_str, [[<FlattenedXmlMapWithXmlNameRequest>
        <KVP>
            <K>a</K>
            <V>A</V>
        </KVP>
