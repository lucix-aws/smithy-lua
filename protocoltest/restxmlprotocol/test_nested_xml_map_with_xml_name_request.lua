-- Code generated by lua-client-codegen. DO NOT EDIT.

-- Generated protocol test file — do not edit
-- Protocol: restXml

local types = require("restxmlprotocol.schemas")
local schema = require("smithy.schema")
local shape_id = require("smithy.shape_id")
local traits = require("smithy.traits")
local http = require("smithy.http")
local h = require("smithy.testing")
local protocol_mod = require("smithy.protocol.restxml")
local protocol = protocol_mod.new()

local service = schema.service({
    id = shape_id.from("aws.protocoltests.restxml", "RestXml"),
    version = "2019-12-16",
    traits = {},
})

describe("NestedXmlMapWithXmlName request", function()
    it("NestedXmlMapWithXmlNameSerializes", function()
        local input = {
            nestedXmlMapWithXmlNameMap = {
            foo = {
            bar = "Baz",
            fizz = "Buzz",
        },
            qux = {
            foobar = "Bar",
            fizzbuzz = "Buzz",
        },
        },
        }
        local operation = schema.operation({
            id = shape_id.from("aws.protocoltests.restxml", "NestedXmlMapWithXmlName"),
            input = types.NestedXmlMapWithXmlNameInput,
            output = schema.new({ type = "structure" }),
            traits = {
                [traits.HTTP] = { method = "POST", path = "/NestedXmlMapWithXmlName" },
            },
        })
        local request, err = protocol:serialize(input, service, operation)
        assert.is_nil(err, "serialize error: " .. tostring(err))
        assert.are.equal("POST", request.method)
        h.assert_url_path(request.url, "/NestedXmlMapWithXmlName")
        h.assert_header(request, "Content-Type", "application/xml")
        local body_str = h.read_body(request.body)
        h.assert_xml_eq(body_str, [[    <NestedXmlMapWithXmlNameRequest>
                <nestedXmlMapWithXmlNameMap>
                    <entry>
                        <OuterKey>foo</OuterKey>
                        <value>
                            <entry>
                                <InnerKey>bar</InnerKey>
                                <InnerValue>Baz</InnerValue>
                            </entry>
                            <entry>
                                <InnerKey>fizz</InnerKey>
                                <InnerValue>Buzz</InnerValue>
                            </entry>
                        </value>
                    </entry>
                    <entry>
                        <OuterKey>qux</OuterKey>
                        <value>
                            <entry>
                                <InnerKey>foobar</InnerKey>
                                <InnerValue>Bar</InnerValue>
                            </entry>
                            <entry>
                                <InnerKey>fizzbuzz</InnerKey>
                                <InnerValue>Buzz</InnerValue>
                            </entry>
                        </value>
                    </entry>
                </nestedXmlMapWithXmlNameMap>
            </NestedXmlMapWithXmlNameRequest>
        ]])
    end)

end)
