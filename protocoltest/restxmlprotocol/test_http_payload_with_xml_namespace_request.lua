...
test("HttpPayloadWithXmlNamespace", function()
    local input = {
        nested = {
        name = "Phreddy",
    },
    ...
    assert_url_path(request.url, "/HttpPayloadWithXmlNamespace")
    assert_header(request, "Content-Type", "application/xml")
    local body_str = read_body(request.body)
    assert_xml_eq(body_str, [[<PayloadWithXmlNamespace xmlns="http://foo.com">
        <name>Phreddy</name>
