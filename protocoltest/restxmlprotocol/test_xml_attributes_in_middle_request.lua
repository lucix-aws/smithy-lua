...
test("XmlAttributesInMiddle", function()
    local input = {
        payload = {
        foo = "Foo",
        attr = "attributeValue",
    ...
    assert_url_path(request.url, "/XmlAttributesInMiddle")
    assert_header(request, "Content-Type", "application/xml")
    local body_str = read_body(request.body)
    assert_xml_eq(body_str, [[<XmlAttributesInMiddlePayloadRequest test="attributeValue">
        <foo>Foo</foo>
        <baz>Baz</baz>
