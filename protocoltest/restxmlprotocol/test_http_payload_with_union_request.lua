...
test("RestXmlHttpPayloadWithUnion", function()
    local input = {
        nested = {
        greeting = "hello",
    },
    ...
    assert_url_path(request.url, "/HttpPayloadWithUnion")
    assert_header(request, "Content-Type", "application/xml")
    local body_str = read_body(request.body)
    assert_xml_eq(body_str, [[<UnionPayload>
        <greeting>hello</greeting>
    </UnionPayload>]])
end)

test("RestXmlHttpPayloadWithUnsetUnion", function()
    local input = {}
    local operation = schema.operation({
        id = shape_id.from("aws.protocoltests.restxml", "HttpPayloadWithUnion"),
        input = types.HttpPayloadWithUnionInput,
