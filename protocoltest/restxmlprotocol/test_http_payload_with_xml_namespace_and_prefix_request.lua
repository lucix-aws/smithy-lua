...
test("HttpPayloadWithXmlNamespaceAndPrefix", function()
    local input = {
        nested = {
        name = "Phreddy",
    },
    ...
    assert_url_path(request.url, "/HttpPayloadWithXmlNamespaceAndPrefix")
    assert_header(request, "Content-Type", "application/xml")
    local body_str = read_body(request.body)
    assert_xml_eq(body_str, [[<PayloadWithXmlNamespaceAndPrefix xmlns:baz="http://foo.com">
        <name>Phreddy</name>
