...
test("RestXmlEndpointTraitWithHostLabel", function()
    local input = {
        label = "bar",
    }
    ...
    assert_url_path(request.url, "/EndpointWithHostLabelOperation")
    local body_str = read_body(request.body)
    assert_xml_eq(body_str, [[<EndpointWithHostLabelOperationRequest>
        <label>bar</label>
