...
test("NoOutputClientAllowsEmptyCbor", function()
    local response = {
        status_code = 200,
        headers = {
        ...
        body = http.string_reader(base64_decode([[v/8=]])),
    }
    local operation = schema.operation({
        id = shape_id.from("smithy.protocoltests.rpcv2Cbor", "NoInputOutput"),
        input = schema.new({ type = "structure" }),
...
test("NoOutputClientAllowsEmptyBody", function()
    local response = {
        status_code = 200,
        headers = {
            ["Content-Type"] = "application/cbor",
