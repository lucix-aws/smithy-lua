...
test("QueryCompatibleRpcV2CborNoCustomCodeError", function()
    local response = {
        status_code = 400,
        headers = {
        ...
        body = http.string_reader(base64_decode([[uQACZl9fdHlwZXgtYXdzLnByb3RvY29sdGVzdHMucnBjdjJjYm9yI05vQ3VzdG9tQ29kZUVycm9yZ21lc3NhZ2ViSGk=]])),
    }
    ...
    assert(err.code == "NoCustomCodeError" or (err.code and err.code:find("NoCustomCodeError", 1, true)),
        "error code: expected NoCustomCodeError, got: " .. tostring(err.code))
    assert_eq(err.message, "Hi", "error message")
end)
