...
test("Ec2Lists", function()
    ...
    assert_form_eq(body_str, [[Action=QueryLists&Version=2020-01-08&ListArg.1=foo&ListArg.2=bar&ListArg.3=baz&ComplexListArg.1.Hi=hello&ComplexListArg.2.Hi=hola]])
end)

test("Ec2EmptyQueryLists", function()
    local input = {
        ListArg = {},
    }
    local operation = schema.operation({
    ...
    assert_header(request, "Content-Type", "application/x-www-form-urlencoded")
    local body_str = read_body(request.body)
    assert_form_eq(body_str, [[Action=QueryLists&Version=2020-01-08]])
end)

test("Ec2ListArgWithXmlNameMember", function()
    local input = {
    ...
    assert_header(request, "Content-Type", "application/x-www-form-urlencoded")
    local body_str = read_body(request.body)
    assert_form_eq(body_str, [[Action=QueryLists&Version=2020-01-08&ListArgWithXmlNameMember.1=A&ListArgWithXmlNameMember.2=B]])
end)

test("Ec2ListMemberWithXmlName", function()
    local input = {
        ListArgWithXmlName = {
        "A",
        "B",
...
test("Ec2ListNestedStructWithList", function()
    local input = {
        NestedWithList = {
        ListArg = {
        "A",
    ...
    assert_header(request, "Content-Type", "application/x-www-form-urlencoded")
    local body_str = read_body(request.body)
    assert_form_eq(body_str, [[Action=QueryLists&Version=2020-01-08&NestedWithList.ListArg.1=A&NestedWithList.ListArg.2=B]])
end)
