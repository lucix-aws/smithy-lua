...
test("RestJsonStreamingTraitsRequireLengthWithBlob", function()
    local input = {
        foo = "Foo",
    ...
    assert_url_path(request.url, "/StreamingTraitsRequireLength")
    assert_header(request, "Content-Type", "application/octet-stream")
    assert_header(request, "X-Foo", "Foo")
    local body_str = read_body(request.body)
    assert_eq(body_str, [[blobby blob blob]], "body")
end)

test("RestJsonStreamingTraitsRequireLengthWithNoBlobBody", function()
    local input = {
        foo = "Foo",
    }
    local operation = schema.operation({
