...
test("RestJsonOmitsNullQuery", function()
    local input = {
        nullValue = nil,
    }
...
test("RestJsonSerializesEmptyQueryValue", function()
    ...
    assert_eq(request.method, "GET", "method")
    assert_url_path(request.url, "/OmitsNullSerializesEmptyString")
    assert_query_param(request.url, "Empty=")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)
