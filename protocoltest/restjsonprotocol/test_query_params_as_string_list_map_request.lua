...
test("RestJsonQueryParamsStringListMap", function()
    local input = {
        qux = "named",
        foo = {
        baz = {
    ...
    assert_url_path(request.url, "/StringListMap")
    assert_query_param(request.url, "corge=named")
    assert_query_param(request.url, "baz=bar")
    assert_query_param(request.url, "baz=qux")
