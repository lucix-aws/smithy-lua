...
test("MediaTypeHeaderInputBase64", function()
    local input = {
        json = "true",
    }
    local operation = schema.operation({
        id = shape_id.from("aws.protocoltests.restjson", "MediaTypeHeader"),
        input = types.MediaTypeHeaderInput,
    ...
    assert_url_path(request.url, "/MediaTypeHeader")
    assert_header(request, "X-Json", "dHJ1ZQ==")
