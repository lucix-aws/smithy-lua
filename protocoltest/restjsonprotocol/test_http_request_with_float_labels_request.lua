...
test("RestJsonSupportsNaNFloatLabels", function()
    local input = {
        float = 0/0,
        double = 0/0,
    }
...
test("RestJsonSupportsInfinityFloatLabels", function()
    ...
    assert(not err, "serialize error: " .. tostring(err))
    assert_eq(request.method, "GET", "method")
    assert_url_path(request.url, "/FloatHttpLabels/Infinity/Infinity")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonSupportsNegativeInfinityFloatLabels", function()
    local input = {
        float = -math.huge,
        double = -math.huge,
    }
