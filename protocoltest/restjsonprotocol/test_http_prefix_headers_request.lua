...
test("RestJsonHttpPrefixHeadersArePresent", function()
    local input = {
        foo = "Foo",
        fooMap = {
        abc = "Abc value",
    ...
    assert_url_path(request.url, "/HttpPrefixHeaders")
    assert_header(request, "x-foo", "Foo")
    assert_header(request, "x-foo-abc", "Abc value")
    assert_header(request, "x-foo-def", "Def value")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonHttpPrefixHeadersAreNotPresent", function()
    local input = {
        foo = "Foo",
        fooMap = {},
    }
...
test("RestJsonHttpPrefixEmptyHeaders", function()
    local input = {
        fooMap = {
        abc = "",
    },
    ...
    assert_eq(request.method, "GET", "method")
    assert_url_path(request.url, "/HttpPrefixHeaders")
    assert_header(request, "x-foo-abc", "")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)
