...
test("RestJsonConstantQueryString", function()
    local input = {
        hello = "hi",
    }
    local operation = schema.operation({
        id = shape_id.from("aws.protocoltests.restjson", "ConstantQueryString"),
    ...
    assert_url_path(request.url, "/ConstantQueryString/hi")
    assert_query_param(request.url, "foo=bar")
    assert_query_param(request.url, "hello")
