...
test("RestJsonConstantAndVariableQueryStringMissingOneValue", function()
    local input = {
        baz = "bam",
    }
    ...
    assert_url_path(request.url, "/ConstantAndVariableQueryString")
    assert_query_param(request.url, "foo=bar")
    assert_query_param(request.url, "baz=bam")
    assert_no_query_param(request.url, "maybeSet")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonConstantAndVariableQueryStringAllValues", function()
    local input = {
        baz = "bam",
        maybeSet = "yes",
    }
    ...
    assert_query_param(request.url, "foo=bar")
    assert_query_param(request.url, "baz=bam")
    assert_query_param(request.url, "maybeSet=yes")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)
