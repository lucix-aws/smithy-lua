...
test("RestJsonHttpWithEmptyStructurePayload", function()
    local input = {}
...
test("RestJsonTestPayloadStructure", function()
    ...
    assert_header(request, "Content-Type", "application/json")
    local body_str = read_body(request.body)
    assert_json_eq(body_str, [[{"data": 25
    }]])
end)

test("RestJsonHttpWithHeadersButNoPayload", function()
    local input = {
        testId = "t-12345",
    }
    local operation = schema.operation({
