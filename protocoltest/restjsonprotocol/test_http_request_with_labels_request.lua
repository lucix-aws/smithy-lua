...
test("RestJsonInputWithHeadersAndAllParams", function()
    local input = {
        string = "string",
        short = 1,
        integer = 2,
    ...
    local operation = schema.operation({
        id = shape_id.from("aws.protocoltests.restjson", "HttpRequestWithLabels"),
        input = types.HttpRequestWithLabelsInput,
    ...
    assert_url_path(request.url, "/HttpRequestWithLabels/string/1/2/3/4.1/5.1/true/2019-12-16T23%3A48%3A18Z")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonHttpRequestLabelEscaping", function()
    local input = {
        string = " %:/?#[]@!$&'()*+,;=😹",
        short = 1,
        integer = 2,
    ...
    assert(not err, "serialize error: " .. tostring(err))
    assert_eq(request.method, "GET", "method")
    assert_url_path(request.url, "/HttpRequestWithLabels/%20%25%3A%2F%3F%23%5B%5D%40%21%24%26%27%28%29%2A%2B%2C%3B%3D%F0%9F%98%B9/1/2/3/4.1/5.1/true/2019-12-16T23%3A48%3A18Z")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)
