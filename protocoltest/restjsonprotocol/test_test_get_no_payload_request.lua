...
test("RestJsonHttpGetWithNoModeledBody", function()
    local input = {}
    local operation = schema.operation({
        id = shape_id.from("aws.protocoltests.restjson", "TestGetNoPayload"),
        input = types.TestGetNoPayloadInput,
    ...
    assert_url_path(request.url, "/no_payload")
    assert_no_header(request, "Content-Length")
...
test("RestJsonHttpGetWithHeaderMemberNoModeledBody", function()
    local input = {
        testId = "t-12345",
    }
    local operation = schema.operation({
    ...
    assert_eq(request.method, "GET", "method")
    assert_url_path(request.url, "/no_payload")
    assert_header(request, "X-Amz-Test-Id", "t-12345")
    assert_no_header(request, "Content-Length")
    assert_no_header(request, "Content-Type")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
