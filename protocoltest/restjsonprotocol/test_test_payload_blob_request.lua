...
test("RestJsonHttpWithEmptyBlobPayload", function()
    local input = {}
...
test("RestJsonTestPayloadBlob", function()
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/blob_payload")
    assert_header(request, "Content-Type", "image/jpg")
    local body_str = read_body(request.body)
    assert_eq(body_str, [[1234]], "body")
end)
