...
test("RestJsonInputAndOutputWithStringHeaders", function()
    local input = {
        headerString = "Hello",
        headerStringList = {
        "a",
    ...
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-String", "Hello")
    assert_header(request, "X-StringList", "a, b, c")
    assert_header(request, "X-StringSet", "a, b, c")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonInputAndOutputWithQuotedStringHeaders", function()
    local input = {
        headerStringList = {
        "b,c",
        "\"def\"",
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-StringList", "\"b,c\", \"\\\"def\\\"\", a")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonInputAndOutputWithNumericHeaders", function()
    local input = {
        headerByte = 1,
        headerShort = 123,
        headerInteger = 123,
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-Byte", "1")
    assert_header(request, "X-Double", "1.1")
    assert_header(request, "X-Float", "1.1")
    assert_header(request, "X-Integer", "123")
    assert_header(request, "X-IntegerList", "1, 2, 3")
    assert_header(request, "X-Long", "123")
    assert_header(request, "X-Short", "123")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonInputAndOutputWithBooleanHeaders", function()
    local input = {
        headerTrueBool = true,
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-Boolean1", "true")
    assert_header(request, "X-Boolean2", "false")
    assert_header(request, "X-BooleanList", "true, false, true")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonInputAndOutputWithTimestampHeaders", function()
    local input = {
        headerTimestampList = {
        1576540098,
        1576540098,
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-TimestampList", "Mon, 16 Dec 2019 23:48:18 GMT, Mon, 16 Dec 2019 23:48:18 GMT")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonInputAndOutputWithEnumHeaders", function()
    local input = {
        headerEnum = "Foo",
        headerEnumList = {
        "Foo",
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-Enum", "Foo")
    assert_header(request, "X-EnumList", "Foo, Bar, Baz")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonInputAndOutputWithIntEnumHeaders", function()
    local input = {
        headerIntegerEnum = 1,
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-IntegerEnum", "1")
    assert_header(request, "X-IntegerEnumList", "1, 2, 3")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonSupportsNaNFloatHeaderInputs", function()
    local input = {
        headerFloat = 0/0,
        headerDouble = 0/0,
    }
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-Double", "NaN")
    assert_header(request, "X-Float", "NaN")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonSupportsInfinityFloatHeaderInputs", function()
    local input = {
    ...
    assert_eq(request.method, "POST", "method")
    assert_url_path(request.url, "/InputAndOutputWithHeaders")
    assert_header(request, "X-Double", "Infinity")
    assert_header(request, "X-Float", "Infinity")
    local body_str = read_body(request.body)
    assert(body_str == "" or body_str == nil, "expected empty body, got: " .. tostring(body_str))
end)

test("RestJsonSupportsNegativeInfinityFloatHeaderInputs", function()
    local input = {
        headerFloat = -math.huge,
        headerDouble = -math.huge,
    }
