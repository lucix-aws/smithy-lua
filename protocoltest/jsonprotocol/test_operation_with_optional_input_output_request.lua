...
test("can_call_operation_with_no_input_or_output", function()
    local input = {}
    ...
    assert_header(request, "X-Amz-Target", "JsonProtocol.OperationWithOptionalInputOutput")
    local body_str = read_body(request.body)
    assert_json_eq(body_str, [[{}]])
end)

test("can_call_operation_with_optional_input", function()
    local input = {
        Value = "Hi",
    }
    local operation = schema.operation({
    ...
    assert_header(request, "X-Amz-Target", "JsonProtocol.OperationWithOptionalInputOutput")
    local body_str = read_body(request.body)
    assert_json_eq(body_str, [[{"Value":"Hi"}]])
end)
