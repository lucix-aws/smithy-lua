...
describe("OperationWithOptionalInputOutput request", function()
    it("can_call_operation_with_no_input_or_output", function()
        local input = {}
        local operation = schema.operation({
            id = shape_id.from("aws.protocoltests.json", "OperationWithOptionalInputOutput"),
            input = types.OperationWithOptionalInputOutputInput,
        ...
        h.assert_header(request, "X-Amz-Target", "JsonProtocol.OperationWithOptionalInputOutput")
        local body_str = h.read_body(request.body)
        h.assert_json_eq(body_str, [[{}]])
    end)

    it("can_call_operation_with_optional_input", function()
        local input = {
            Value = "Hi",
        }
        local operation = schema.operation({
        ...
        h.assert_header(request, "X-Amz-Target", "JsonProtocol.OperationWithOptionalInputOutput")
        local body_str = h.read_body(request.body)
        h.assert_json_eq(body_str, [[{"Value":"Hi"}]])
    end)
