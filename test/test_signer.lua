...
describe("signer", function()
    ...
    it("sigv4: session token is added", function()
        ...
        assert(not err)
        assert.are.equal("AQoDYXdzEJr...", signed.headers["X-Amz-Security-Token"])
    ...
    it("sigv4: host header is set from URL", function()
        ...
        local signed, err = signer.sign(request, creds, props)
        assert(not err)
        assert.are.equal("sts.us-east-1.amazonaws.com", signed.headers["Host"])
    end)
    ...
    it("sigv4: deterministic signing", function()
        ...
        assert.are.equal(s2.headers["Authorization"], s1.headers["Authorization"], "signatures should be deterministic")
    end)
end)
