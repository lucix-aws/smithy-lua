...
function M:serialize(input, _service, operation)
   ...
   if payload_name then
      ...
      elseif payload_schema.type == stype.STRUCTURE or payload_schema.type == stype.UNION then
         ...
         local id_any = target_schema.id
         local id_tbl = id_any
         local root = (xml_name_trait and xml_name_trait.name) or
         (id_tbl and id_tbl.name) or
         payload_name
         local err
      ...
      if has_body then
         ...
         local id_any = schema.id
         local id_tbl = id_any
         local root = xml_name and xml_name.name or (id_tbl and id_tbl.name) or "root"
