...
local function decode_schema_value(v, schema, codec)
   ...
   if st == stype.STRUCTURE then
      ...
      for name, member_schema in pairs(members) do
         ...
         local entry = { name, member_schema }
         by_wire[wn] = entry
      end
      local result = {}
      ...
      for name, ms in pairs(members) do
         if result[name] == nil then
            ...
            elseif ms:trait(t.REQUIRED) then
               ...
               elseif mt == stype.BYTE or mt == stype.SHORT or mt == stype.INTEGER or
                  mt == stype.LONG or mt == stype.FLOAT or mt == stype.DOUBLE or
                  mt == stype.INT_ENUM or mt == stype.TIMESTAMP then                   result[name] = 0
...
function M._parse_iso8601(s)
   ...
   local time_tbl = { year = tonumber(y), month = tonumber(mo), day = tonumber(d),
hour = tonumber(h), min = tonumber(mi), sec = tonumber(sec), isdst = false, }
   local tt = os.time(time_tbl)
...
function M._parse_http_date(s)
   ...
   local tt_tbl = { year = tonumber(year), month = months[mon] or 1, day = tonumber(day),
hour = tonumber(h), min = tonumber(mi), sec = tonumber(sec), isdst = false, }
   local epoch = os.time(tt_tbl)
   local utc_dt = os.date("!*t", 0)
   local local_dt = os.date("*t", 0)
   local utc_offset = os.time(utc_dt) - os.time(local_dt)
