-- smithy-lua runtime: request/response logging interceptor
-- Usage:
--   local logging = require("smithy.interceptor.logging")
--   local client = s3.new({ interceptors = { logging.new() } })

local http = require("smithy.http")

local M = {}
local Interceptor = {}
Interceptor.__index = Interceptor

function M.new()
    return setmetatable({}, Interceptor)
end

function Interceptor:read_before_transmit(ctx)
    local req = ctx.request
    io.stderr:write("~~ SDK Request ~~\n")
    io.stderr:write(req.method .. " " .. req.url .. "\n")
    for k, v in pairs(req.headers) do
        io.stderr:write(k .. ": " .. v .. "\n")
    end
    if req.body then
        local body, err = http.read_all(req.body)
        if body then
            io.stderr:write("\n" .. body .. "\n")
            req.body = http.string_reader(body)
        end
    end
    io.stderr:write("\n")
end

function Interceptor:read_after_transmit(ctx)
    local resp = ctx.response
    io.stderr:write("~~ SDK Response ~~\n")
    io.stderr:write(tostring(resp.status_code) .. "\n")
    for k, v in pairs(resp.headers) do
        io.stderr:write(k .. ": " .. v .. "\n")
    end
    io.stderr:write("\n")
end

return M
